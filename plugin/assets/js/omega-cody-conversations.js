(function(window, document) {
	...
	var copyThreadButton = document.getElementById('omega-cody-copy-thread');
	var copyThreadSource = document.getElementById('omega-cody-copy-thread-source');
	var copyThreadStatus = document.getElementById('omega-cody-copy-thread-status');
	...
	function setCopyThreadStatus(value) {
		if (!copyThreadStatus) {
			return;
		}

		copyThreadStatus.textContent = value;
	}

	function markCopyThreadSuccess() {
		if (!copyThreadButton) {
			return;
		}

		copyThreadButton.disabled = false;
		setCopyThreadStatus(getText('threadCopied', 'Conversation copied'));
	...
	function markCopyThreadFailure() {
		if (copyThreadButton) {
			copyThreadButton.disabled = false;
		}
	...
	function copyThreadToClipboard() {
		...
		if (!copyThreadButton || !copyThreadSource) {
			return;
		}

		transcript = copyThreadSource.value;
		if ('' === transcript) {
	...
	if (copyThreadButton && copyThreadSource) {
		copyThreadButton.addEventListener('click', copyThreadToClipboard);
	}
